...
function logWebhookEvent(
  ...
  if (logData.email_addresses && Array.isArray(logData.email_addresses)) {
    logData.email_addresses = logData.email_addresses.map((addr) => ({
      ...addr,
      email_address:
        typeof addr.email_address === "string" && addr.email_address
...
function validateUserData(data: unknown): {
  ...
  if (
    ...
    if (
      ...
      !primaryEmail.email_address.includes("@")
    ) {
  ...
  return {
    ...
    firstName:
      userData.first_name && typeof userData.first_name === "string"
        ...
        : null,
    lastName:
      userData.last_name && typeof userData.last_name === "string"
        ? userData.last_name
        : null,
  };
}
      ...
      switch (type) {
        case "user.created": {
          ...
          try {
            ...
            logWebhookEvent(
              "error",
              "Database error during user and credits creation",
              {
                userId: data.id,
                error:
            ...
            return Response.json(
              {
                success: false,
                error: "Failed to create user and credits record",
              } as ClerkWebhookResponse,
              { status: 500 },
            );
          }
        ...
        case "user.updated": {
          ...
          if (!validation.isValid) {
            ...
            logWebhookEvent(
              "info",
              "Proceeding with partial user update despite validation warnings",
            );
          }
        ...
        default:
          logWebhookEvent("warn", `Unhandled webhook type: ${type}`, {
            eventType: type,
          });
