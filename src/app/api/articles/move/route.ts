...
async function generateArticleContentInline(articleId: string) {
  console.log(
    "Starting article generation by calling generate API for article:",
    articleId,
  );
  try {
    // Call the generate API endpoint instead of duplicating logic
    const generateResponse = await fetch(
      `${API_BASE_URL}/api/articles/generate`,
      {
        method: "POST",
  ...
  try {
    ...
    if (!isValidStatusTransition(currentArticle.status, newStatus)) {
      return NextResponse.json(
        {
          error:
            "Invalid status transition. Articles can only move forward in the workflow.",
        },
        { status: 400 },
      );
    }
    ...
    if (newStatus === "published") {
      ...
      fetch(
        `${process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000"}/api/webhooks/deliver`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        console.error(
          "Failed to trigger webhook delivery for article",
          articleId,
          ":",
          error,
        );
    ...
    if (newStatus === "generating") {
      try {
        ...
        generateArticleContentInline(articleId.toString()).catch(
          (error: unknown) => {
