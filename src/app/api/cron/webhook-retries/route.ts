    ...
    for (const delivery of retryDeliveries) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (
            nextAttempt < delivery.max_attempts &&
            shouldRetry(null, responseStatus)
          ) {
        ...
        if (
          nextAttempt < delivery.max_attempts &&
          shouldRetry(fetchError, responseStatus)
        ) {
          const retryDelay = calculateRetryDelay(nextAttempt + 1);
...
export async function GET() {
  try {
    ...
    console.log(
      `Webhook retries completed: processed ${result.processedCount}, succeeded ${result.successCount}, failed ${result.failedCount}`,
    );
