...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!projectId || !providers || providers.length === 0 || !base?.text?.trim()) {
    ...
    if (providers.includes("reddit")) {
      if (!reddit?.subreddit?.trim() || !reddit?.title?.trim()) {
        return NextResponse.json({ error: "Reddit requires subreddit and title" }, { status: 400 });
      }
    }
    ...
    if (providers.includes("x")) {
      ...
      if (!xText) {
        return NextResponse.json({ error: "X requires text content" }, { status: 400 });
      }
      if (xText.length > 280) {
        return NextResponse.json({ error: "X text must be 280 characters or less" }, { status: 400 });
      }
    }
    ...
    const publishedResults: Array<{ id: number; provider: Provider; status: "published" }> = [];
    const errors: string[] = [];
      ...
      try {
        if (provider === "reddit") {
          ...
          if (updatedMetadata.redditTokens?.[projectId.toString()]) {
            ...
            await clerk.users.updateUserMetadata(userId, { privateMetadata: updatedMetadata });
          }
          ...
          if (!conn.refreshToken) {
            errors.push("X refresh token is missing - please reconnect your X account");
            continue;
          }
          ...
          // Get access token and submit post
          try {
            const { accessToken, newRefreshToken } = await ensureXAccessToken(conn.refreshToken);
            ...
            if (newRefreshToken) {
              ...
              if (updatedMetadata.xTokens?.[projectId.toString()]) {
                updatedMetadata.xTokens[projectId.toString()]!.refreshToken = newRefreshToken;
                updatedMetadata.xTokens[projectId.toString()]!.lastUsedAt = new Date().toISOString();
                await clerk.users.updateUserMetadata(userId, { privateMetadata: updatedMetadata });
              }
            }
            ...
            if (errorMessage.includes("401") || errorMessage.includes("invalid_client") || errorMessage.includes("invalid_grant")) {
              errors.push("X authentication expired - please reconnect your X account in settings");
            } else {
    ...
    if (publishedResults.length === 0 && errors.length > 0) {
      return NextResponse.json({ error: `All posts failed: ${errors.join(", ")}` }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, data: publishedResults } satisfies SocialPostResponse);
  } catch (error) {
