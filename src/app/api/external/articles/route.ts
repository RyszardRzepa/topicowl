...
// GET /api/external/articles - list published articles using API key
// Auth: Authorization: Bearer <API_KEY>
// Query params: projectId (optional, number)
export async function GET(request: Request) {
  try {
    const authHeader = request.headers.get('authorization');
  if (!authHeader?.toLowerCase().startsWith('bearer ')) {
      return NextResponse.json({ error: 'Missing or invalid Authorization header (expected Bearer token)' }, { status: 401 });
    }
    ...
    db
      ...
      .catch((err) => {
        console.error('Failed to update API key lastUsedAt for keyId:', keyRecord.id, err);
      });
    ...
    if (projectIdParam) {
      ...
      if (requestedProjectId !== keyRecord.projectId) {
        return NextResponse.json({ error: 'API key does not have access to specified project' }, { status: 403 });
      }
    }
    ...
    const rows = await db
      ...
      .from(articles)
      .where(and(eq(articles.projectId, keyRecord.projectId), eq(articles.status, 'published')))
