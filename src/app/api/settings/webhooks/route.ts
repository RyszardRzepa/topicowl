  ...
  try {
    ...
    if (process.env.NODE_ENV === "production") {
      ...
      if (
        ...
        return {
          isValid: false,
          error: "Cannot use private/local URLs in production",
        };
      }
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response: WebhookSettingsResponse = {
      ...
      data: {
        ...
        webhookEvents: Array.isArray(projectRecord.webhookEvents)
          ...
          : ["article.published"],
        hasSecret: !!(
          projectRecord.webhookSecret && projectRecord.webhookSecret.length > 0
        ),
      },
    };
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Failed to retrieve webhook settings",
      } as WebhookSettingsResponse,
      { status: 500 },
    );
  }
    ...
    const response: WebhookSettingsResponse = {
      ...
      data: {
        ...
        webhookEvents: Array.isArray(updatedProject.webhookEvents)
          ...
          : ["article.published"],
        hasSecret: !!(
          updatedProject.webhookSecret &&
          updatedProject.webhookSecret.length > 0
        ),
      },
    };
