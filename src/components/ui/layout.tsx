  ...
  return (
    <div className={cn("min-h-screen bg-white", className)} {...props}>
      <ContentContainer fullWidth={fullWidth}>
        {title && (
          <header className="mb-8 border-b border-stone-200 py-8">
            <h1 className="font-inter mb-2 text-2xl font-semibold text-stone-700">
              {title}
            </h1>
            {description && (
              <p className="font-inter text-stone-500">{description}</p>
            )}
          </header>
        )}
        <main>{children}</main>
