...
const transformUnsplashImage = (
  photo: UnsplashPhoto,
): Omit<UnsplashImage, "relevanceScore"> => ({
  id: photo.id,
...
const transformPexelsImage = (
  photo: PexelsPhoto,
): Omit<PexelsImage, "relevanceScore"> => ({
  id: photo.id,
...
const fetchUnsplashSearch = async (
  query: string,
...
Keywords: ${keywords.join(", ")}`;
  ...
  // Search Unsplash
  for (const q of effectiveQueries.slice(0, 2)) {
    // Limit Unsplash searches to avoid rate limits
    searchPromises.push(
  ...
  const unsplashTransformed = allUnsplashImages
    .slice(0, unsplashCount)
    .map((img) => ({
 ...
 */
export async function performImageSelectionLogic(
