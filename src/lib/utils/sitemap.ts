...
export const sitemapUrlSchema = z.string().url().refine((url) => {
  // Allow various sitemap URL formats
  return url.includes('sitemap') || url.endsWith('.xml');
}, {
  message: "URL must be a valid sitemap (should contain 'sitemap' or end with '.xml')"
});
...
export function validateSitemapUrl(url: string): { isValid: boolean; error?: string } {
  try {
    sitemapUrlSchema.parse(url);
    return { isValid: true };
  } catch (error) {
    if (error instanceof z.ZodError) {
      return { isValid: false, error: error.errors[0]?.message ?? 'Invalid sitemap URL' };
    }
